...
class ConvNet:
    ...
    def build_network(self, built_for_training=False):  # build a network similar with SRCNN, a fully-convolutional netowrk for channel noise estimation
        ...
        x_in = tf.compat.v1.placeholder(tf.float32, [None, self.net_config.feature_length])  # input data
        ...
        for layer in range(self.net_config.total_layers):  # construct layers
            ...
            if built_for_training:
                ...
                self.conv_filter[layer] = tf.compat.v1.get_variable(name=self.conv_filter_name[layer], shape=[self.net_config.filter_sizes[layer], 1, in_channels, out_channels],
                                                          dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
                self.bias[layer] = tf.compat.v1.get_variable(name=self.bias_name[layer], shape=[out_channels],
                                                   dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
                ...
                self.conv_filter[layer] = tf.Variable(tf.random.normal([self.net_config.filter_sizes[layer], 1, in_channels, out_channels], 0, 1, tf.float32), dtype=tf.float32,
                                                      name=self.conv_filter_name[layer])
                self.bias[layer] = tf.Variable(tf.random.normal([out_channels], 0, 1, tf.float32), dtype=tf.float32, name=self.bias_name[layer])
    ...
    def train_network(self, model_id):
        ...
        y_label = tf.compat.v1.placeholder(tf.float32, [None, self.net_config.label_length])
        ...
        # SGD_Adam
        train_step = tf.compat.v1.train.AdamOptimizer().minimize(training_loss)
        ...
        print(self.train_config.epoch_num)
