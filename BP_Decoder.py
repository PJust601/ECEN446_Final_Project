...
class BP_NetDecoder:
    ...
    def one_bp_iteration(self, xe_v2c_pre_iter, H_sumC_to_V, H_sumV_to_C, xe_0):
        ...
        xe_sum_log_real = tf.matmul(H_sumC_to_V, tf.math.log(1e-8 + tf.abs(xe_tanh)))
        xe_sum_log_complex = tf.complex(xe_sum_log_real, xe_sum_log_img)
        xe_product = tf.math.real(tf.exp(xe_sum_log_complex))
    ...
    def build_network(self): # build the network for one BP iteration
        ...
        dec_out = tf.transpose(tf.math.floordiv(1-tf.to_int32(tf.sign(bp_out_llr)), 2))
